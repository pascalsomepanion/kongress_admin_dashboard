...
type TGroup={tnId:number;tn:Row['teilnehmer'];buchungen:Row[];rNr:string|null;versendet:string|null;allBezahlt:boolean}
...
export default function RechnungenPage(){
  ...
  const groups:TGroup[]=Object.values(
    rows.reduce((acc,r)=>{
      if(!acc[r.teilnehmer_id])acc[r.teilnehmer_id]={tnId:r.teilnehmer_id,tn:r.teilnehmer,buchungen:[],rNr:null,versendet:null,allBezahlt:false}
  ...
  async function createPreview(g:TGroup){
    ...
    const html=buildRechnungHTML({
      ...
      anrede:anredeText,
      empfaenger_name:`${tn.vorname} ${tn.nachname}`,
      empfaenger_strasse:`${tn.strasse} ${tn.hausnummer}`,
      empfaenger_plz_ort:`${tn.postleitzahl} ${tn.stadt}`,
      empfaenger_land:tn.land,
      empfaenger_kennung:`ÖÄK Nr.: ${tn.oeak_nr}`,
      positionen:aktiveBuchungen.map(b=>({bezeichnung:b.kurse.titel,menge:1,einzelpreis:b.gebuchter_preis})),
      mwst_typ:'mit_mwst',
      bezahlt:g.allBezahlt,
      kongress_name:k.name,
      kongress_jahr:k.jahr,
      intro_text:`Vielen Dank für Ihr Interesse am Sportmedizin Kongress St. Christoph am Arlberg ${k.jahr}.\n\nFür die Teilnahme an den u.a. Kursen dürfen wir folgende Rechnung stellen:`,
    })
    setPreview(html)
    setCreating(g)
  }
  ...
  async function send(g:TGroup){
    ...
    const html=buildRechnungHTML({
      rechnungsnummer:g.rNr,datum:new Date().toLocaleDateString('de-AT'),anrede:anredeText,
      empfaenger_name:`${tn.vorname} ${tn.nachname}`,empfaenger_strasse:`${tn.strasse} ${tn.hausnummer}`,
      empfaenger_plz_ort:`${tn.postleitzahl} ${tn.stadt}`,empfaenger_land:tn.land,empfaenger_kennung:`ÖÄK Nr.: ${tn.oeak_nr}`,
      positionen:aktiveBuchungen.map(b=>({bezeichnung:b.kurse.titel,menge:1,einzelpreis:b.gebuchter_preis})),
            ...
            {groups.map(g=>{
              ...
              return(
                <tr key={g.tnId} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 font-bold whitespace-nowrap">€ {total.toFixed(2)}</td>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              {(['Damen und Herren','Frau','Herr'] as const).map(a=>(
                <button key={a} onClick={()=>setAnrede(a)} className={`px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${anrede===a?'bg-[#FFBF00] border-[#FFBF00] text-black':'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{a==='Damen und Herren'?'Sehr geehrte Damen und Herren':`Sehr geehrte${a==='Herr'?'r':''} ${a}`}</button>
              ))}
            </div>
          </div>
          <div className="bg-gray-50 rounded-xl p-4 mb-4">
            <p className="text-xs font-semibold text-gray-500 mb-2">Gebuchte Kurse (nicht storniert)</p>
            ...
            <div className="flex justify-between font-bold mt-2 pt-2 border-t border-gray-200">
              <span>Gesamt (Brutto inkl. 20% MwSt.)</span>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div><h2 className="font-bold text-base">Vorschau — {previewNr}</h2><p className="text-xs text-gray-400 mt-0.5">Bitte überprüfen — nach dem Speichern öffnet sich der Druckdialog</p></div>
