...
export default function RechnungenPage(){
  ...
  function buildStornoHtml(g:TGroup,stornoNr:string,origNr:string,stornierte:Buchung[],erstattung:number=0,einbehalt:number=0,typ:string='kostenlos'):string{
    ...
    const rows=stornierte.map((b,i)=>`<tr><td style="border:1px solid #ccc;padding:6px 10px;font-size:10px">${i+1}.</td><td style="border:1px solid #ccc;padding:6px 10px;font-size:10px">${b.kurse.titel}</td><td style="border:1px solid #ccc;padding:6px 10px;text-align:right;font-size:10px">−€ ${b.gebuchter_preis.toFixed(2)}</td></tr>`).join('')
    return`<!DOCTYPE html><html lang="de"><head><meta charset="utf-8"><title>Stornorechnung ${stornoNr}</title><style>@page{size:A4;margin:15mm 20mm 25mm 20mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#111;min-height:297mm;display:flex;flex-direction:column;}@media print{body{-webkit-print-color-adjust:exact}}</style></head><body>
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8mm;padding-bottom:5mm;border-bottom:2px solid #FFBF00">
  <div style="display:flex;align-items:center;gap:8px">
    ...
    <div><div style="font-size:11px;font-weight:700;color:#111">Prof. h.c. Univ.-Doz. Dr. Günther Neumayr</div>
    <div style="font-size:9px;color:#777">Österreichische Gesellschaft für Sportmedizin und Prävention</div></div>
  </div>
...
<div style="margin-bottom:6mm;font-size:10px;line-height:1.8"><div style="font-weight:bold">${g.tn.vorname} ${g.tn.nachname}</div><div>${g.tn.strasse} ${g.tn.hausnummer}</div><div>${g.tn.postleitzahl} ${g.tn.stadt}</div><div style="color:#555">ÖÄK Nr.: ${g.tn.oeak_nr}</div></div>
<div style="margin-bottom:4mm"><div style="font-size:15px;font-weight:bold;color:#dc2626">Stornorechnung / Gutschrift</div><div style="font-size:10px;color:#555;margin-top:2px">Nr.: <strong>${stornoNr}</strong> · zur Rechnung ${origNr}</div></div>
...
<table style="width:100%;border-collapse:collapse;font-size:10px;margin-bottom:5mm"><thead><tr style="background:#FFBF00;color:#111"><th style="border:1px solid #ccc;padding:6px 10px;text-align:left;width:5%">Pos.</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:left">Bezeichnung</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:right;width:18%">Betrag</th></tr></thead><tbody>${rows}</tbody></table>
<div style="display:flex;justify-content:flex-end;margin-bottom:5mm"><table style="width:260px">${typ==='50prozent'?`<tr><td style="font-size:10px;padding:3px 0;color:#555">Gesamtbetrag</td><td style="text-align:right;font-size:10px;padding:3px 0">-€ ${brutto.toFixed(2)}</td></tr><tr><td style="font-size:10px;padding:3px 0;color:#dc2626">Stornogebühr 50%</td><td style="text-align:right;font-size:10px;padding:3px 0;color:#dc2626">+€ ${einbehalt.toFixed(2)}</td></tr>`:''}${erstattung>0?`<tr><td style="font-size:10px;padding:3px 0;color:#555">Netto</td><td style="text-align:right;font-size:10px;padding:3px 0">-€ ${netto.toFixed(2)}</td></tr><tr><td style="font-size:10px;padding:3px 0;color:#555">Ust. 20% inkl.</td><td style="text-align:right;font-size:10px;padding:3px 0">-€ ${mwst.toFixed(2)}</td></tr>`:''}<tr style="border-top:2px solid #dc2626"><td style="font-weight:bold;font-size:12px;padding:5px 0;color:#dc2626">${erstattung>0?'Rückerstattung':'Keine Erstattung'}</td><td style="text-align:right;font-weight:bold;font-size:13px;padding:5px 0;color:#dc2626">${erstattung>0?`-€ ${erstattung.toFixed(2)}`:'€ 0.00'}</td></tr></table></div>${erstattung>0?`<p style="font-size:10px;color:#555;margin-bottom:8mm">EUR ${erstattung.toFixed(2)} wird auf Ihr Konto zurücküberwiesen.</p>`:`<p style="font-size:10px;font-weight:bold;color:#dc2626;margin-bottom:8mm">Gemäß AGB erfolgt keine Rückerstattung.</p>`}
<div style="font-size:10px;line-height:1.9;margin-top:8mm"><p>Mit sportlichen Grüßen</p><br><br><div style="display:inline-block;text-align:center"><img src="/unterschrift.png" style="height:15mm;width:auto;display:block;margin-bottom:3px" onerror="this.style.display='none'"/><p style="font-weight:bold;font-style:italic">Prof. h.c. Univ.-Doz. Dr. Günther Neumayr</p><p>Kongresspräsident</p></div></div>
<div style="margin-top:auto;width:100%;padding-top:4mm;border-top:1px solid #ccc;display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:9px;color:#555;margin-top:10mm"><div><div style="font-weight:bold;margin-bottom:2px">Prof. h.c. Univ.-Doz. Dr. Günther Neumayr</div><div>Michaelsgasse 20</div><div>9900 Lienz, Österreich</div><div>UID: ATU 61957546</div></div><div style="text-align:center"><div style="font-weight:bold;margin-bottom:2px">Bankverbindung</div><div>Inhaber: Günther Neumayr</div><div>IBAN: AT67 1912 0500 9922 3610</div><div>BIC: SPBAATWW · Bank99</div></div><div style="text-align:right"><div style="font-weight:bold;margin-bottom:2px">Kontakt</div><div>Tel.: 04852 61952-52</div><div>E-Mail: info@sportmedizin-arlberg.at</div><div>Website: www.sportmedizin-arlberg.at</div></div>
</div>
</body></html>`
  }
