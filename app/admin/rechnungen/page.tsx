...
// Berechnet Stornogebühr basierend auf Frist
function calcStornoErstattung(betrag:number, k:Kongress): {erstattung:number; einbehalt:number; typ:'kostenlos'|'50prozent'|'keine'} {
  ...
  if(heute <= fuenfzigBis) return {erstattung:betrag*0.5, einbehalt:betrag*0.5, typ:'50prozent'}
...
export default function RechnungenPage(){
  ...
  function getHistorie(buchungen:Buchung[]):{rNr:string|null;buchungen:Buchung[];allBezahlt:boolean;hasOffen:boolean;hatBezahlteUndStorniert:boolean;versendetAm:string|null}[]{
  ...
  function buildStornoHtml(g:TGroup,stornoNr:string,origNr:string,stornierte:Buchung[],erstattung:number,einbehalt:number,stornoTyp:string):string{
    ...
    const netto=erstattung/1.2
    const mwst=erstattung-netto
    const heute=new Date().toLocaleDateString('de-AT')
    const stornoText = stornoTyp==='kostenlos'
      ? 'Da die kostenlose Stornofrist noch nicht abgelaufen ist, erstatten wir Ihnen den vollen Betrag.'
      : stornoTyp==='50prozent'
      ? `Da die kostenlose Stornofrist abgelaufen ist, behalten wir gemäß unseren AGB 50% (€ ${einbehalt.toFixed(2)}) ein.`
      : 'Leider ist die Stornofrist abgelaufen. Gemäß unseren AGB kann keine Erstattung erfolgen.'
    ...
    const posTR=stornierte.map((b,i)=>`
      <tr>
        ...
        <td style="border:1px solid #ccc;padding:6px 10px;text-align:right;font-size:10px">−${b.gebuchter_preis.toFixed(2)}</td>
        ${stornoTyp==='50prozent'?`<td style="border:1px solid #ccc;padding:6px 10px;text-align:right;font-size:10px;color:#dc2626">−${(b.gebuchter_preis*0.5).toFixed(2)}</td>`:`<td style="border:1px solid #ccc;padding:6px 10px;text-align:right;font-size:10px;color:#dc2626">−${stornoTyp==='kostenlos'?b.gebuchter_preis.toFixed(2):'0.00'}</td>`}
      </tr>`).join('')
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8mm">
      <div></div>
      <div style="text-align:right"><img src="/logo.svg" style="height:18mm;width:auto;display:block;margin-left:auto;margin-bottom:6px"/>
        <div style="font-size:10px;line-height:1.8;color:#333"><div>${heute}</div><div>E-Mail: info@sportmedizin-arlberg.at</div></div>
      </div>
    </div>
    ...
    <div style="margin-bottom:5mm;font-size:10px;line-height:1.7">
      ...
      <p>hiermit stornieren wir folgende Position(en) aus unserer Rechnung ${origNr}:</p><br>
      <p>${stornoText}</p>
    </div>
    <table style="width:100%;border-collapse:collapse;font-size:10px;margin-bottom:5mm">
      <thead><tr style="background:#f0f0f0">
        ...
        <th style="border:1px solid #ccc;padding:6px 10px;text-align:right;width:13%">Ursprungspreis</th>
        <th style="border:1px solid #ccc;padding:6px 10px;text-align:right;width:13%">Erstattung</th>
      </tr></thead>
      <tbody>${posTR}</tbody>
    </table>
    ${stornoTyp!=='keine'?`
    <div style="display:flex;justify-content:flex-end;margin-bottom:5mm">
      <table style="width:240px;border-collapse:collapse">
        ${stornoTyp==='50prozent'?`<tr><td style="border:none;font-size:10px;padding:4px 0;color:#555">Ursprungsbetrag</td><td style="border:none;text-align:right;font-size:10px;padding:4px 0">−${bruttoGesamt.toFixed(2)}</td></tr>
        <tr><td style="border:none;font-size:10px;padding:4px 0;color:#555">Stornogebühr 50%</td><td style="border:none;text-align:right;font-size:10px;padding:4px 0;color:#dc2626">+${einbehalt.toFixed(2)}</td></tr>`:''}
        <tr><td style="border:none;font-size:10px;padding:4px 0;color:#555">Nettobetrag Erstattung</td><td style="border:none;text-align:right;font-size:10px;padding:4px 0">−${netto.toFixed(2)}</td></tr>
        ...
        <tr style="border-top:2px solid #dc2626">
          <td style="border:none;font-weight:bold;font-size:12px;padding:6px 0 4px;color:#dc2626">Rückerstattungsbetrag</td>
          <td style="border:none;text-align:right;font-weight:bold;font-size:14px;padding:6px 0 4px;color:#dc2626">−${erstattung.toFixed(2)}</td>
        </tr>
      </table>
    </div>
    <p style="font-size:10px;color:#555;margin-bottom:8mm">Der Rückerstattungsbetrag von EUR ${erstattung.toFixed(2)} wird auf Ihr Konto zurücküberwiesen.</p>
  ...
  async function saveTeilstorno(erstattung:number, einbehalt:number, stornoTyp:string){
    ...
    for(const b of Array.from(teilstornoSelected).map(id=>teilstornoBuchungen.find(b=>b.id===id)!)){
    ...
    setPreviewHtml(null);setTeilstornoGroup(null);setTeilstornoBuchungen([]);setTeilstornoSelected(new Set())
    await load(k.id);setSaving(false)
  }
  ...
  async function sendZahlungserinnerung(g:TGroup){
    ...
    await fetch('/api/send-zahlungserinnerung',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
      email:g.tn.email,vorname:g.tn.vorname,nachname:g.tn.nachname,
  ...
  function openTeilstorno(g:TGroup, rNr:string){
    ...
    setTeilstornoGroup(g);setTeilstornoRechNr(rNr);setTeilstornoBuchungen(buchungen);setTeilstornoSelected(new Set())
  }
  ...
  return(
    <div>
            ...
            {groups.map((g,i)=>{
              ...
              return(
                <div key={g.tnId} className={i>0?'border-t border-gray-100':''}>
                  ...
                  {isOpen&&(
                    <div className="bg-[#FFFDF5] border-t border-[#FFE082]/50 px-6 py-4 space-y-4">
                      {/* Zahlungserinnerung */}
                      {hatAusstehend&&(
                        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center justify-between">
                          ...
                          <Btn size="sm" variant="outline" disabled={erinnerungSending===g.tnId} onClick={()=>sendZahlungserinnerung(g)}>
                            {erinnerungSending===g.tnId?'Sendet…':'📧 Zahlungserinnerung senden'}
                          </Btn>
                        </div>
                      )}
                          ...
                          <div key={hi} className="border border-gray-200 rounded-xl overflow-hidden">
                            <div className={`px-4 py-3 flex items-center justify-between ${h.rNr?'bg-gray-50':'bg-amber-50'}`}>
                              ...
                              <div className="flex gap-1.5 flex-wrap">
                                ...
                                {/* Senden: nur wenn bezahlt */}
                                {h.rNr&&bezahlt&&(
                                  <Btn size="sm" variant="outline" disabled={sending===h.rNr} onClick={async()=>{
                                    const html=buildHtml(g,h.rNr!,h.buchungen,'Damen und Herren',bezahlt)
                                    await sendEmail(g,h.rNr!,html)
                                  }}>
                                    {sending===h.rNr?'Sendet…':versendet?'📧 Erneut senden':'📧 Senden'}
                                  </Btn>
                                )}
                                {h.rNr&&!bezahlt&&<span className="text-[10px] text-gray-400 italic">Senden erst nach Zahlungseingang</span>}
                                {/* Teilstorno: nur wenn bezahlt und Rechnung vorhanden */}
                                {h.rNr&&bezahlt&&aktivBuchungen.length>0&&(
                                  <Btn size="sm" variant="danger" onClick={()=>openTeilstorno(g,h.rNr!)}>
                                    🔴 Teilstorno
                                  </Btn>
                                )}
      ...
      {teilstornoGroup&&!previewHtml&&(
        <Modal title={`Teilstorno — ${teilstornoGroup.tn.nachname} ${teilstornoGroup.tn.vorname}`} onClose={()=>{setTeilstornoGroup(null);setTeilstornoBuchungen([]);setTeilstornoSelected(new Set())}} wide>
          <div className="space-y-4">
            {/* Fristen anzeigen */}
            {k&&(
              <div className="grid grid-cols-3 gap-3 text-xs">
                {[
                  {label:'Kostenlos bis',date:k.storno_kostenlos_bis,color:'green'},
                  {label:'50% Gebühr bis',date:k.storno_50_bis,color:'amber'},
                  {label:'Danach',text:'Keine Erstattung',color:'red'},
                ].map(f=>{
                  const heute=new Date()
                  const d=f.date?new Date(f.date):null
                  const aktiv=d?heute<=d:false
                  return(
                    <div key={f.label} className={`rounded-xl p-3 border ${aktiv?'bg-green-50 border-green-200':'bg-gray-50 border-gray-200'}`}>
                      <p className="font-bold text-gray-700">{f.label}</p>
                      <p className={aktiv?'text-green-700':'text-gray-400'}>{f.date?new Date(f.date).toLocaleDateString('de-AT'):f.text}</p>
                      {d&&<p className={`text-[10px] mt-1 font-semibold ${aktiv?'text-green-600':'text-red-500'}`}>{aktiv?'✓ Noch gültig':'✗ Abgelaufen'}</p>}
                    </div>
                  )
                })}
              </div>
              ...
              <div className={`rounded-xl p-4 border ${stornoCalc.typ==='keine'?'bg-red-50 border-red-200':stornoCalc.typ==='50prozent'?'bg-amber-50 border-amber-200':'bg-green-50 border-green-200'}`}>
                ...
                <div className="space-y-1.5 text-sm">
                  ...
                  <div className={`flex justify-between font-bold text-base border-t pt-2 mt-2 ${stornoCalc.typ==='keine'?'text-red-700':stornoCalc.typ==='50prozent'?'text-amber-700':'text-green-700'}`}>
                    <span>{stornoCalc.typ==='keine'?'Keine Erstattung':'Erstattung an Teilnehmer'}</span>
                    <span>€ {stornoCalc.erstattung.toFixed(2)}</span>
                  </div>
                  {stornoCalc.einbehalt>0&&<div className="flex justify-between text-gray-500 text-xs"><span>Einbehalt (Stornogebühr)</span><span>€ {stornoCalc.einbehalt.toFixed(2)}</span></div>}
                </div>
                {stornoCalc.typ==='keine'&&<p className="text-xs text-red-600 mt-2 font-medium">⚠ Die Stornofrist ist abgelaufen. Es erfolgt keine Rückerstattung.</p>}
              </div>
            )}
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <Btn variant="danger" disabled={teilstornoSelected.size===0} onClick={async()=>{
                ...
                const html=buildStornoHtml(teilstornoGroup,nr,teilstornoRechNr??'',selectedTeilstornoBuchungen,stornoCalc.erstattung,stornoCalc.einbehalt,stornoCalc.typ)
                setPreviewHtml(html);setPreviewNr(nr);setPreviewMode('storno')
            ...
            <div className={`flex items-center justify-between px-6 py-4 border-b ${previewMode==='storno'?'bg-red-50':''}`}>
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {previewMode==='new'?'Neu erstellt — bitte überprüfen':previewMode==='storno'?'Bitte überprüfen vor dem Speichern':'Gespeicherte Rechnung'}
                </p>
              </div>
              <div className="flex gap-3">
                <Btn variant="outline" onClick={()=>{setPreviewHtml(null);if(previewMode!=='existing'){setCreating(null);setTeilstornoGroup(null)}}}>← Schließen</Btn>
                {previewMode==='new'&&creating&&<Btn onClick={saveRechnung} disabled={saving}>{saving?'Speichert…':'✓ Speichern & Drucken'}</Btn>}
                {previewMode==='storno'&&teilstornoGroup&&stornoCalc&&(
                  <Btn variant="danger" onClick={()=>saveTeilstorno(stornoCalc.erstattung,stornoCalc.einbehalt,stornoCalc.typ)} disabled={saving}>
                    {saving?'Speichert…':'✓ Stornorechnung speichern'}
                  </Btn>
                )}
                {previewMode==='existing'&&(
                  <>
                    <Btn variant="outline" onClick={()=>{const win=window.open('','_blank');if(win){win.document.write(previewHtml!);win.document.close();setTimeout(()=>win.print(),600)}}}>🖨 Drucken</Btn>
                    {(()=>{
                      const g=groups.find(g=>g.buchungen.some(b=>b.rechnungsnummer===previewNr))
                      const bez=g?.buchungen.filter(b=>b.rechnungsnummer===previewNr&&b.zahlungsstatus!=='storniert').every(b=>b.zahlungsstatus==='bezahlt')??false
                      return bez&&g?<Btn variant="outline" disabled={sending===previewNr} onClick={async()=>{if(previewHtml)await sendEmail(g,previewNr,previewHtml)}}>{sending===previewNr?'Sendet…':'📧 Erneut senden'}</Btn>:null
                    })()}
                  </>
                )}
              </div>
