...
export default function RechnungenPage(){
  ...
  function buildStornoHtml(g:TGroup,stornoNr:string,origNr:string,stornierte:Buchung[],erstattung:number=0,einbehalt:number=0,typ:string='kostenlos'):string{
    ...
    return`<!DOCTYPE html><html lang="de"><head><meta charset="utf-8"><title>Stornorechnung ${stornoNr}</title><style>@page{size:A4;margin:15mm 20mm 20mm 20mm}*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#111}@media print{body{-webkit-print-color-adjust:exact}}</style></head><body> <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8mm"><div></div><div style="text-align:right"><img src="/logo.svg" style="height:18mm;width:auto;display:block;margin-left:auto;margin-bottom:6px"/><div style="font-size:10px;line-height:1.8;color:#333"><div>${new Date().toLocaleDateString('de-AT')}</div><div>Bearbeiterin: Dr. iur. Mara Neumayr, MBL</div><div>E-Mail: info@sportmedizin-arlberg.at</div></div></div></div> <div style="margin-bottom:7mm;font-size:10px;line-height:1.8"><div style="font-weight:bold">${g.tn.vorname} ${g.tn.nachname}</div><div>${g.tn.strasse} ${g.tn.hausnummer}</div><div>${g.tn.postleitzahl} ${g.tn.stadt}</div><div style="color:#555;margin-top:2px">ÖÄK Nr.: ${g.tn.oeak_nr}</div></div> <div style="margin-bottom:4mm"><div style="font-size:16px;font-weight:bold;color:#dc2626">Stornorechnung / Gutschrift</div><div style="font-size:11px;font-weight:bold;margin-top:2px">zur Rechnung ${origNr} — ${k.name} ${k.jahr}</div></div> <div style="margin-bottom:5mm;font-size:10px">Stornorechnung-Nr.: <strong>${stornoNr}</strong></div> <div style="margin-bottom:5mm;font-size:10px;line-height:1.7"><p>Sehr geehrte Damen und Herren,</p><br><p>hiermit stornieren wir unsere Rechnung ${origNr} teilweise und erstatten Ihnen folgende Beträge:</p></div> <table style="width:100%;border-collapse:collapse;font-size:10px;margin-bottom:5mm"><thead><tr style="background:#f0f0f0"><th style="border:1px solid #ccc;padding:6px 10px;text-align:left;width:5%">Pos.</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:left">Bezeichnung</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:center;width:8%">Menge</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:left;width:8%">Einheit</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:right;width:13%">Einzelpreis</th><th style="border:1px solid #ccc;padding:6px 10px;text-align:right;width:13%">Gesamtpreis</th></tr></thead><tbody>${posTR}</tbody></table> <div style="display:flex;justify-content:flex-end;margin-bottom:5mm"><table style="width:220px;border-collapse:collapse"><tr><td style="border:none;font-size:10px;padding:4px 0;color:#555">Bruttobetrag</td><td style="border:none;text-align:right;font-size:10px;padding:4px 0;color:#dc2626">−${brutto.toFixed(2)}</td></tr><tr><td style="border:none;font-size:10px;padding:4px 0;color:#555">Ust. 20% inkl.</td><td style="border:none;text-align:right;font-size:10px;padding:4px 0;color:#dc2626">−${mwst.toFixed(2)}</td></tr><tr><td style="border:none;font-weight:bold;font-size:12px;padding:6px 0 4px;border-top:2px solid #dc2626;color:#dc2626">Rückerstattungsbetrag</td><td style="border:none;text-align:right;font-weight:bold;font-size:14px;padding:6px 0 4px;border-top:2px solid #dc2626;color:#dc2626">−${brutto.toFixed(2)}</td></tr></table></div> <p style="font-size:10px;color:#555;margin-bottom:8mm">Der Rückerstattungsbetrag von EUR ${brutto.toFixed(2)} wird auf Ihr Konto zurücküberwiesen.</p> <div style="font-size:10px;line-height:1.9;margin-top:8mm"><p>Mit sportlichen Grüßen</p><br><br><img src="/unterschrift.png" style="height:15mm;width:auto;display:block;margin:0 auto 2mm auto" onerror="this.style.display:'none'"/><p style="font-weight:bold;font-style:italic">Prof. h.c. Univ.-Doz. Dr. Günther Neumayr</p><p>Kongresspräsident</p></div> <div style="margin-top:auto;padding-top:5mm;border-top:1px solid #ccc;display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;font-size:9px;color:#555;position:fixed;bottom:10mm;left:20mm;right:20mm"><div><div style="font-weight:bold;margin-bottom:3px">Prof. h.c. Univ.-Doz. Dr. Günther Neumayr</div><div>Michaelsgasse 20, 9900 Lienz</div><div>UID: ATU 61957546</div></div><div><div style="font-weight:bold;margin-bottom:3px">Bankverbindung</div><div>IBAN: AT67 1912 0500 9922 3610</div><div>BIC: SPBAATWW · Bank99</div></div><div><div style="font-weight:bold;margin-bottom:3px">Kontakt</div><div>Tel.: 04852 61952-52</div><div>info@sportmedizin-arlberg.at</div></div></div> </body></html>`
  }
