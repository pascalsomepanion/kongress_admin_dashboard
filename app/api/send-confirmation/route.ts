...
export async function POST(req:NextRequest){
  try{
    ...
    const{email,vorname,nachname,kongress_name,kongress_jahr,kongress_datum,kongress_start,kongress_ende,iban,bic,kontoinhaber,kontakt_email,kurs_titel,gesamtbetrag,oeak_nr,ist_oegsmp_mitglied,fruehbucher_bis,storno_kostenlos_bis,storno_50_bis,sekretariat_zeiten}=body
    ...
    const kursRows=alleKurse.map(k=>{
      ...
      return `<tr><td style="padding:5px 10px;border:1px solid #e5e7eb;font-size:12px">${k.label}</td><td style="padding:5px 10px;border:1px solid #e5e7eb;text-align:center;font-size:14px">${gebucht?'<span style="color:#16a34a;font-weight:bold">&#10003;</span>':''}</td></tr>`
    }).join('')

    const zeiten = sekretariat_zeiten ? sekretariat_zeiten.replace(/\n/g,'<br>') : 'Sonntag: 15:00-17:00 und 18:00-20:00 Uhr<br>Montag bis Freitag: Vormittag 07:45-08:15 und 09:15-09:45 Uhr / Nachmittag 16:30-17:00 und 18:15-18:45 Uhr'
...
<div style="background:#FFBF00;padding:20px 30px;border-radius:8px 8px 0 0">
  <p style="font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:rgba(0,0,0,.45);margin:0 0 4px">Anmeldebestaetigung</p>
...
<div style="border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;padding:28px 30px">
  ...
  <p style="margin-top:12px"><strong>Oeffnungszeiten des Kongress-Sekretariates:</strong><br>${zeiten}</p>
  ...
  <p style="font-size:12px;color:#666">Bitte beachten Sie, dass die definitive Anmeldung erst mit dem vollstaendigen Zahlungseingang eintritt.</p>

  <hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0">

  <h3 style="font-size:13px;font-weight:700;margin-bottom:10px">Kursuebersicht:</h3>
  <table style="border-collapse:collapse;width:100%;margin-bottom:8px">
    <thead><tr style="background:#f5f5f5"><th style="padding:6px 10px;border:1px solid #e5e7eb;text-align:left;font-size:11px;font-weight:700">Kurs</th><th style="padding:6px 10px;border:1px solid #e5e7eb;text-align:center;font-size:11px;font-weight:700;width:80px">gebucht</th></tr></thead>
    <tbody>${kursRows}</tbody>
  </table>

  <hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0">

  <h3 style="font-size:13px;font-weight:700;margin-bottom:8px">Ihre persoenlichen Daten:</h3>
  <table style="font-size:12px;border-collapse:collapse">
    ...
    <tr><td style="color:#888;padding:3px 10px 3px 0">OEGSMP Mitglied</td><td style="font-weight:600">${ist_oegsmp_mitglied?'Ja':'Nein'}</td></tr>
  </table>
  ...
  <p><strong>Aenderungen und nachtraegliche Buchungen:</strong> Bei Aenderungswuenschen bitten wir Sie, direkt mit unserem Kongresssekretariat unter <a href="mailto:${kontakt_email}" style="color:#B45309">${kontakt_email}</a> in Kontakt zu treten.</p>
  <p style="margin-top:10px"><strong>Fruehbucherbonus:</strong> Gilt bei Zahlungseingang bis ${fruehbucher_bis}.</p>
  <p style="margin-top:10px"><strong>Stornogebuehr:</strong> Bis ${storno_kostenlos_bis}: kostenlos. Bis ${storno_50_bis}: 50% Rueckerstattung. Ab danach keine Rueckerstattung.</p>
  <p style="margin-top:10px"><strong>Anwesenheit:</strong> Wird kontrolliert und ist Voraussetzung fuer die Anerkennung der DFPs.</p>
  <p style="margin-top:10px"><strong>DFP:</strong> Volle Punkte nur bei vollstaendiger Anwesenheit. Steuerliche Absetzbarkeit bei mind. 8 Stunden Nachweis.</p>
  <p style="margin-top:10px"><strong>Zimmerreservierung:</strong> <a href="https://www.skiakademie.at" style="color:#B45309">www.skiakademie.at</a></p>

  <hr style="border:none;border-top:1px solid #e5e7eb;margin:20px 0">

  <p>Wir freuen uns auf einen erfolgreichen Kongress und verbleiben mit sportlichem Gruss!</p>
  <p style="margin-top:16px"><strong>Prof. h.c. Univ.-Doz. Dr. Guenther Neumayr</strong><br><span style="color:#888;font-size:12px">Kongresspräsident</span></p>
