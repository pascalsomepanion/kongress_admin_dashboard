  ...
  return (
    <main className="min-h-screen bg-[#F7F6F3]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            ...
            <table className="w-full text-xs border-collapse">
              <thead><tr className="border-b-2 border-gray-100">
                <th className="pb-2 text-[10px] font-bold uppercase tracking-wide text-gray-400 text-left">Kurs</th>
                <th className="pb-2 text-[10px] font-bold uppercase tracking-wide text-gray-400 text-left">Datum & Uhrzeit</th>
                <th className="pb-2 text-[10px] font-bold uppercase tracking-wide text-gray-400 text-right">Frühbucher</th>
                <th className="pb-2 text-[10px] font-bold uppercase tracking-wide text-gray-400 text-right">Normaltarif</th>
              </tr></thead>
              <tbody>
                ...
                {blockKurse.map(k=>(
                  <tr key={k.id} className="border-b border-gray-50">
                    ...
                    <td className="py-2 text-gray-500 text-[11px]">{k.uhrzeit ?? k.wochentag_datum}</td>
                ...
                <tr><td colSpan={4} className="pt-3 pb-1 text-[10px] font-bold uppercase tracking-wide text-gray-400">Praxis- & Theorieseminare</td></tr>
                {psKurse.map((k,i)=>{
                  const ts=tsKurse[i]
                  return (
                    <tr key={k.id} className="border-b border-gray-50">
                      <td className="py-2 text-gray-700">{k.titel} / {ts?.titel}</td>
                      <td className="py-2 text-gray-500 text-[11px]">{k.uhrzeit ?? k.wochentag_datum}</td>
                      <td className="py-2 text-right font-semibold text-gray-800">€ {k.fruehbucher_preis}</td>
                      <td className="py-2 text-right font-semibold text-amber-700">€ {k.spaetbucher_preis}</td>
                    </tr>
        ...
        {step === 'confirm' && <>
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4"><div className="w-1 h-4 bg-[#FFBF00] rounded-full"/><h2 className="text-[10px] font-bold tracking-widest uppercase text-gray-400">Persönliche Daten</h2></div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            {Array.from(selected).map(id => {
              ...
              return (
                <div key={id} className="py-2.5 border-b border-gray-100 last:border-0">
                  <div className="flex justify-between text-sm"><span className="font-semibold text-gray-800">{k.titel}</span><span className="font-bold">€ {p.toFixed(2)}</span></div>
                  {k.uhrzeit && <p className="text-xs text-gray-400 mt-0.5">{k.uhrzeit}</p>}
                </div>
              )
            })}
